                ...
                {strip}
                    <table id="related_contact" class="display">
                        ...
                        <tr id="rel_{$rel.id}" class="{cycle values="odd-row,even-row"} row-relationship {if $rel.membership_id}row-highlight{/if}">
                            <td>
                                <a href="{crmURL p='civicrm/contact/view/rel' q="action=view&reset=1&selectedChild=rel&cid=`$rel.cid`&id=`$rel.id`"}">{$rel.relation}</a>{if $rel.is_permission_a_b}&nbsp;<span class="crm-marker" title="permissioned relationship">*</span>{/if}
                            </td>
